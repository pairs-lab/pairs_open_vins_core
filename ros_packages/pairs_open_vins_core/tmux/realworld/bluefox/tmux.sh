...
'
  'Core' 'waitForTime; roslaunch pairs_uav_core core.launch platform_config:=`rospack find pairs_uav_deployment`/config/pairs_uav_system/$UAV_TYPE.yaml world_config:=config/world_config.yaml custom_config:=./config/custom_config.yaml network_config:=./config/network_config.yaml
'
  'AutoStart' 'waitForHw; roslaunch pairs_uav_autostart automatic_start.launch
'
  'OpenVins' 'waitForHw; roslaunch pairs_open_vins_core bluefox.launch
'
  'Republisher' 'waitForHw; roslaunch pairs_vins_republisher vins_republisher_openvins_bluefox.launch
'
  'Imu' 'waitForHw; roslaunch pairs_serial vio_imu.launch
'
  'ImuFilter' 'waitForHw; roslaunch pairs_vins_imu_filter filter_icm_42688.launch
'
